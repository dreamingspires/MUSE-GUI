...
def get_commodity(
    ...
    return Commodity(
        ...
        commodity_prices=[get_commodity_price(time=x, region=y) for x in years for y in regions],
    )
...
class CommodityView(TwoColumnMixin, BaseView):
    ...
    def _update_info(self, window, model: Commodity):
        ...
        _values = sorted(
            [[p.time, p.region_name, p.value] for p in _prices],
            key=lambda x: x[0]
        )
            ...
            if new_name != _commodity.commodity:
                ...
                for d in deps:
                    if len(deps[d]):
                        ...
                        raise SaveException() from RuntimeError('Changing name is not supported for commodities already associated with resources')
    ...
    def _handle_add_commodity(self, window):
        ...
        commodity = sg.popup_get_text(
            'Please enter name of commodity to add', 'Add Commodity',
            'New Commodity',
        )
