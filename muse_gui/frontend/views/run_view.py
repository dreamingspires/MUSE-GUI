...
class CarbonMarketInfo(BaseWidget):
    ...
    def layout(self, prefix) -> List[List[Element]]:
        if not self._layout:
            self.prefix = prefix
            self._activate = self._activate_maker(key=self._prefixf('activate_carbon_market'))
            self._method_options = self._method_options_maker(key=self._prefixf('method_options'))
                ...
                [
                    sg.Frame(
                        ...
                        layout = self._budget_table.layout(self._prefixf('budget')) + [
                            [ _commodities],
                            [
                                sg.Checkbox('Control undershoot', key=self._prefixf("undershoot"))
                            ],
                            [
                                sg.Checkbox('Control overshoot', key=self._prefixf("overshoot"))
                            ],
                            [
        ...
        if not self._layout:
            ...
            self._layout = [
                ...
                [
                    ...
                    sg.Text(':', auto_size_text=True),
                    sg.Input('', size=(8, 1), justification='right', key=self._prefixf('interest_rate')),
                ],
            ] + _form_layout
...
class RunView(BaseView):
    def __init__(self, model: Datastore):
        ...
        self.model = model.run_settings or RunModel(
            ...
            time_framework=[
                int(x) for x in self._datastore.available_year.list()
            ],
            interest_rate=None,
        )
    ...
    def layout(self, prefix):
        if not self._layout:
            ...
            [
                sg.Push(), sg.Button('Run Muse', key = self._prefixf('run'), size = (20,2)), sg.Push()
            ]
        ]
    ...
    def _patch_budget(self):
        ...
        _budget_map = {
            int(x[0]): x[1]
            for x in _budget_values if x
        }

        _new_budget = [
            [x, _budget_map[x]]
            if x in _budget_map else [x, '']
    ...
    def update(self, window):
        ...
        available_years = [self._datastore.available_year.read(x).year for x in self._datastore.available_year.list()]
        ...
        self._carbon_market_commodities = [x for x in self._carbon_market_commodities if x in available_commodities]
    ...
    def __call__(self, window, event, values):
        ...
        elif _event == 'budget' or self._carbon_market._budget_table.should_handle_event(event):
    ...
    def _handle_run(self, window, values):
        ...
        if _state:
            ...
            _budget = [
                x[1] for x in _cm_values['budget'] if x and x[1]
            ]
            ...
            _values['carbon_budget_control'] = CarbonMarket(
                **_cm_values,
                budget = _budget,
                commodities=self._carbon_market_commodities if self._carbon_market_commodities else None
            )
    ...
    def _handle_edit_commodities(self, window):
        ...
        self._carbon_market.set_commodities(
            window,
            self._carbon_market_commodities
        )
